...
bribeResult.addEventListener("click", function(e){
  ...
  var needsMore = '<div class="front"><p>' + $countrySelect.name + ' will need to pay a bribe to FIFA in the amount of ' + Math.floor(rand * $countrySelect.exchange * inflation()).toLocaleString() + ' ' + $countrySelect.currency + '. Remember, FIFA is based in Switzerland and they prefer to receive bribes in francs. So you\'ll need to give FIFA ' + Math.floor(rand * inflation()).toLocaleString() + ' francs.</p><p>' + $countrySelect.name + ' only has ' + $countrySelect.numStadiums + ' stadiums with a capacity of 60,000+ people. ' + $countrySelect.name + ' will need to build ' + stadiumCount() + ' new stadiums in order to host the World Cup. The cost to build ' + stadiumCount() + ' stadiums is only ' + (stadiumCount() * 1200000000).toLocaleString() + ' francs.</p> <button>Want to pick a different country?</button></div>';
  var hasEnough = '<div class="front"><p>' + $countrySelect.name + ' will need to pay a bribe to FIFA in the amount of ' + Math.floor(rand * $countrySelect.exchange * inflation()).toLocaleString() + ' ' + $countrySelect.currency + '. Remember, FIFA is based in Switzerland and they prefer to receive bribes in francs. So you\'ll need to give FIFA ' + Math.floor(rand * inflation()).toLocaleString() + ' francs.</p><p>' + $countrySelect.name + ' has ' + $countrySelect.numStadiums + ' stadiums with a capacity of 60,000+ people. Thats good news because you need at least 12 stadiums to host a World Cup. ' + $countrySelect.name + ' wont need to build new stadiums. So theres that.</p> <button>Want to pick a different country?</button></div>';
